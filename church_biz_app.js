...
function renderAdmin(){
  ...
  if(adminSection==='stats'){
    ...
    body='<div style="font-size:12px;color:var(--t2);margin-bottom:10px">총 '+S.members.length+'명</div>';
    ...
    body='<div style="font-size:12px;color:var(--t2);margin-bottom:10px">총 '+S.biz.length+'개</div>';
...
function doLogin(){
  ...
  // Firebase 로드 중이면 잠시 대기 후 재시도
  if(S.members.length===0){
    err.textContent='데이터 로딩 중... 잠시 후 다시 시도해주세요';err.style.display='block';
    loadFromFirebase().then(function(){
      var m=S.members.find(function(x){return x.phone===phone&&x.pw===pw;});
      if(m){err.style.display='none';S.user=m;if(document.getElementById('rememberMe').checked){try{localStorage.setItem('savedLogin',JSON.stringify({phone:phone,pw:pw}));}catch(e){}}document.getElementById('loginBtn').textContent=m.name+' '+m.role;checkAdminTab();closeM('loginM');renderMypage();}
      else{err.textContent='전화번호 또는 비밀번호가 맞지 않아요';err.style.display='block';}
    });
    return;
  }
  var m=S.members.find(function(x){return x.phone===phone&&x.pw===pw;});
  ...
  err.style.display='none';
  S.user=m;
  if(document.getElementById('rememberMe').checked){try{localStorage.setItem('savedLogin',JSON.stringify({phone:phone,pw:pw}));}catch(e){}}
  document.getElementById('loginBtn').textContent=m.name+' '+m.role;
  checkAdminTab();
  closeM('loginM'); renderMypage();
}
